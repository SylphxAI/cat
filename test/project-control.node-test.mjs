...
test("project manifest is the vendor-neutral GroundAtlas control file", () => {
	...
	assert.equal(manifest.project.id, "cat")
	assert.equal(manifest.project.repository, "https://github.com/SylphxAI/cat")
	...
	assert.ok(manifest.truth.runbooks.includes("SECURITY.md"))
	assert.ok(
...
test("Doctrine adapter remains Sylphx-specific and release boundary is explicit", () => {
	...
	assert.equal(doctrine.project.repo, "SylphxAI/cat")
	assert.equal(doctrine.project.lifecycle, "production")
	assert.ok(
...
test("release workflow grants publish identity permission through the caller", () => {
...
test("root package keeps direct publish disabled", () => {
