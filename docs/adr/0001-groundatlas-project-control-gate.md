...
CI will dogfood the released GroundAtlas package/action (`groundatlas@0.1.3` and `SylphxAI/groundatlas@v0.1.3`) on pull requests, merge groups, and main pushes. The gate must prove that GroundAtlas selects `project.manifest.json`, treats `.doctrine/project.json` only as an adapter, reports the repository as adopted under strict fleet policy, and preserves the Markdown scorecard evidence.

## Consequences

- Generated `.groundatlas*`, fleet JSON, and fleet Markdown reports are evidence/navigation only, never source of truth.
